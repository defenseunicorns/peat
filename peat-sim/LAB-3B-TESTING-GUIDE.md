...
docker exec clab-<name>-peer-1 /usr/local/bin/peat-sim --help
```
...
### Implementation
- `peat-mesh/src/flat_mesh.rs` - Core coordinator
- `peat-sim/src/main.rs` - Integration (flat_mesh_mode)
- `peat-sim/generate-flat-mesh-peat-topology.py` - Topology generator

### Testing
- `quick-test-lab3b.sh` - 5-node validation
- `test-lab3b-peat-mesh.sh` - Full test suite
...
Review core code:
```bash
cat ../peat-mesh/src/flat_mesh.rs
```

Test the coordinator directly:
```bash
cd ../peat-mesh && cargo test flat_mesh
```
