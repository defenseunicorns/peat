...
preflight_checks() {
    ...
    # Check Docker image
    if ! docker image inspect peat-sim-node:latest >/dev/null 2>&1; then
        echo "❌ Docker image peat-sim-node:latest not found"
        errors=$((errors + 1))
    fi
...
e8-performance-tests-quick:
	...
	@cd peat-sim && SKIP_IMAGE_CHECK=1 ./run-e8-performance-suite.sh

# Modify run-e8-performance-suite.sh
if [ "${SKIP_IMAGE_CHECK:-0}" != "1" ]; then
    if ! docker image inspect peat-sim-node:latest >/dev/null 2>&1; then
        echo "❌ Error: peat-sim-node:latest not found"
        exit 1
    fi
fi
```
...
# Add to Makefile
e8-test-quick:
	@cd peat-sim && ./run-e8-performance-suite.sh --filter "100mbps,10mbps" --modes "mode1"
	# Runs only: 3 architectures × 2 bandwidths × 1 mode = 6 tests (~4 minutes)

e8-test-256kbps-only:
	@cd peat-sim && ./run-e8-performance-suite.sh --filter "256kbps"
...
## Prerequisites
- Docker with peat-sim-node:latest image
