...
#
# Issue #519: https://github.com/kitplummer/peat/issues/519
#
# Usage:
...
RESULTS_BASE_DIR="/work/peat-sim-results/parity-tests"
...
for tag in ditto automerge; do
    if ! docker images | grep -q "peat-sim.*${tag}"; then
        log_warning "peat-sim:${tag} image not found, will try peat-sim-node:latest"
    fi
done
...
run_backend_test() {
    ...
    if [[ "$BACKEND" == "automerge" ]]; then
        ...
        export PEAT_SECRET_KEY="aGl2ZS10ZXN0LWZvcm1hdGlvbi1zZWNyZXQta2V5LTA="
        export PEAT_OFFLINE_TOKEN=""
        export PEAT_SHARED_KEY=""
    else
        if [ -f "../.env" ]; then
            set -a
            source ../.env
