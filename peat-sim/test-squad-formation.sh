...
if ! docker image inspect peat-sim-node:latest &> /dev/null; then
    echo -e "${RED}✗ peat-sim-node:latest image not found${NC}"
    echo "  Build it with: docker build -f peat-sim/Dockerfile -t peat-sim-node:latest ."
    exit 1
fi
