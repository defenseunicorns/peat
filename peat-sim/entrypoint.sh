...
# This allows all containers to be deployed before starting the PEAT service
...
# For ditto backend, verify PEAT credentials are set
...
if [ "$USE_TRADITIONAL" != "true" ] && [ "$USE_PRODUCER_ONLY" != "true" ] && [ "$USE_P2P_MESH" != "true" ] && [ "$BACKEND" != "automerge" ]; then
    if [ -z "$PEAT_APP_ID" ]; then
        echo "[${NODE_ID}] ERROR: PEAT_APP_ID not set"
        exit 1
    fi
    if [ -z "$PEAT_OFFLINE_TOKEN" ]; then
        echo "[${NODE_ID}] ERROR: PEAT_OFFLINE_TOKEN not set"
        exit 1
    fi
    # Accept either PEAT_SECRET_KEY or PEAT_SHARED_KEY (credentials.rs handles both)
    if [ -z "$PEAT_SECRET_KEY" ] && [ -z "$PEAT_SHARED_KEY" ]; then
        echo "[${NODE_ID}] ERROR: PEAT_SECRET_KEY or PEAT_SHARED_KEY not set"
        exit 1
    fi
fi
...
if [ "$USE_P2P_MESH" = "true" ] || [ "$MODE" = "p2p_mesh" ]; then
    ...
    # Use peat-sim binary which properly uses the protocol API for hierarchical aggregation
    # This avoids coupling to Ditto-specific SDK details
    export MODE="hierarchical"  # Ensure MODE environment variable is set
    exec /usr/local/bin/peat-sim $ARGS
else
    echo "[${NODE_ID}] Running PEAT Protocol Simulation"
    exec /usr/local/bin/peat-sim $ARGS
fi
