...
# Source .env for credentials (DITTO_*, PEAT_* env vars)
if [[ -f "../.env" ]]; then
    set -a
    source "../.env"
    set +a
...
if [[ "$BACKEND" == "automerge" ]]; then
    # Check for peat-sim:automerge image
    if ! docker images "peat-sim:automerge" --format "{{.Repository}}" | grep -q "peat-sim"; then
        echo "ERROR: peat-sim:automerge image not found"
        echo "Build with: RUSTFLAGS=\"\" docker build -f peat-sim/Dockerfile -t peat-sim:automerge --build-arg BACKEND=automerge ."
        exit 1
    fi
fi
echo "Image: OK (peat-sim:${IMAGE_TAG})"
