...
RESULTS_BASE_DIR="/work/peat-sim-results"
...
preflight_checks() {
    ...
    if ! docker images | grep -q "peat-sim-node"; then
        log_error "peat-sim-node:latest image not found"
    ...
    if [ "$NEEDS_PARITY" = true ]; then
        ...
        if [ -x "${SCRIPT_DIR}/test-lab4-parity.sh" ]; then
            if "${SCRIPT_DIR}/test-lab4-parity.sh" --quick; then
                ...
                echo ""
                echo "Options:"
                echo "  1. Investigate parity results in /work/peat-sim-results/parity-tests/"
