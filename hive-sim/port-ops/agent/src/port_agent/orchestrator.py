"""
Phase 1b / Phase 3 Orchestrator — multi-agent simulation runner.
...
Multi-berth: Use ``--berths 2`` to run 2 independent berth instances.
Each berth gets its own hold, container queue, team state, and entity naming
scoped to the berth (e.g., crane-b1h1-1, crane-b2h1-1).
Yard tractors are shared across berths via a yard manager.
"""
...
class AgentSpec:
    ...
    model: str | None = None
    berth_num: int = 0  # 0 = unscoped (shared), 1+ = berth-scoped
...
def parse_agent_composition(
    ...
    Letters: c=crane, w=operator, t=tractor, s=scheduler, a=aggregator,
             b=berth_manager, x=sensor.

    When ``num_berths > 1``, the composition is duplicated per berth with
    berth-scoped entity naming: ``crane-b1h1-1``, ``crane-b2h1-1``, etc.
    Tractors are shared across berths (berth_num=0) and only instantiated once.
    Each berth also gets its own berth_manager automatically.
    """
    ...
    for berth_idx in range(1, num_berths + 1):
        berth_prefix = f"b{berth_idx}h1" if num_berths > 1 else ""
        sensor_idx = 0
        ...
        for count_str, letter in pairs:
            ...
            # Tractors are shared across berths — only create once (on berth 1)
            if letter == "t" and num_berths > 1 and berth_idx > 1:
                continue

            # Tractors are shared (yard resources) — no berth prefix
            is_shared = (letter == "t" and num_berths > 1)
            use_prefix = berth_prefix if not is_shared else ""

            for i in range(1, count + 1):
                if letter == "x":
                    base_id = _SENSOR_IDS[sensor_idx % len(_SENSOR_IDS)]
                    node_id = f"{base_id}-{use_prefix}" if use_prefix else base_id
                    sensor_idx += 1
                elif id_pattern and "{n}" in id_pattern:
                    base_id = id_pattern.replace("{n}", str(i))
                    node_id = f"{base_id}-{use_prefix}" if use_prefix else base_id
                else:
                    base_id = id_pattern  # singleton like hold-agg-3, scheduler-1
                    node_id = f"{base_id}-{use_prefix}" if use_prefix else base_id

                berth_num = berth_idx if not is_shared else 0
        ...
        # Auto-add a berth_manager per berth when running multi-berth
        # (unless user already specified 'b' in composition)
        if num_berths > 1:
            has_berth_manager = any(l == "b" for _, l in pairs)
            if not has_berth_manager:
                bm_id = f"berth-mgr-{berth_idx}"
                specs.append(AgentSpec(
                    node_id=bm_id,
                    role="berth_manager",
                    persona="berth-manager",
                    provider=provider,
                    model=model,
                    berth_num=berth_idx,
                ))
...
class Orchestrator:
    """
    Multi-agent orchestrator for Phase 1a / Phase 3.

    Creates one shared HiveStateStore, initializes state, then runs
    multiple AgentLoop instances as concurrent asyncio tasks.

    Multi-berth: When ``config.num_berths > 1``, each berth gets its own
    hold, container queue, and team state. Entity naming is berth-scoped
    (e.g., crane-b1h1-1). Tractors are shared across berths.
    """
    ...
    def _berth_hold_id(self, berth_num: int) -> str:
        """Return the hold_id for a given berth number."""
        if self.config.num_berths <= 1:
            return self.config.hold_id
        return f"hold-b{berth_num}"

    def _berth_id(self, berth_num: int) -> str:
        """Return the berth identifier for a given berth number."""
        if self.config.num_berths <= 1:
            return self.config.berth
        return f"berth-{berth_num}"
    ...
    def initialize_state(self):
        ...
        # Initialize per-berth state
        berth_count = max(1, self.config.num_berths)
        for berth_num in range(1, berth_count + 1):
            hold_id = self._berth_hold_id(berth_num)
            berth_id = self._berth_id(berth_num)

            # Create container queue per berth
            ...
            # Create team state per berth
            create_team_state(self.store, hold_id)
            team_doc = self.store.get_document("team_summaries", f"team_{hold_id}")

            # Initialize entity documents for agents in this berth
            for spec in self.config.agents:
                # Skip agents not in this berth (0 = shared, handled separately)
                if spec.berth_num != berth_num and spec.berth_num != 0:
                    continue
                # Shared agents (berth_num=0) are initialized only on the first berth pass
                if spec.berth_num == 0 and berth_num > 1:
                    continue

                self._init_agent_entity(
                    spec, hold_id, berth_id, team_doc, containers,
                )

            # Create transport queue per berth
        ...
        logger.info(
            ...
            f"{self.config.queue_size} containers/berth, "
            f"{berth_count} berth(s)"
        )

    def _init_agent_entity(
        self,
        spec: AgentSpec,
        hold_id: str,
        berth_id: str,
        ...
        """Initialize entity document for a single agent spec."""
        from port_agent_bridge.hive_state import (
            create_crane_entity,
            create_operator_entity,
            create_tractor_entity,
        ...
        hold_num = int(hold_id.split("-")[-1]) if hold_id.split("-")[-1].isdigit() else self.config.hold_num
        ...
        if spec.role == "crane":
            ...
            # First operator in each berth gets hazmat cert
            is_first_op = spec.node_id.endswith("-1") or spec.node_id.endswith("-1-b1h1")
            op_config = {
                "proficiency": "expert",
    ...
    def create_agents(self, personas_dir: Path):
        ...
        for spec in self.config.agents:
            ...
            # Determine the hold_id for this agent's berth
            if spec.berth_num > 0:
                agent_hold_id = self._berth_hold_id(spec.berth_num)
                agent_berth_id = self._berth_id(spec.berth_num)
            else:
                # Shared agents (tractors) use the first berth's hold
                agent_hold_id = self._berth_hold_id(1)
                agent_berth_id = self._berth_id(1)
            ...
            bridge = BridgeAPI(
                ...
                node_id=spec.node_id,
                role=spec.role,
                hold_id=agent_hold_id,
                berth_id=agent_berth_id,
            )
    ...
    async def run(self) -> dict[str, list[CycleMetrics]]:
        ...
        logger.info(
            f"\n{'='*60}\n"
            f"  HIVE Port Operations — {berth_count} Berth(s)\n"
            f"  Agents: {', '.join(s.node_id for s in self.config.agents)}\n"
            f"  Queue: {self.config.queue_size} containers/berth "
        ...
        # Emit vessel approach spatial events (one per berth)
        for berth_num in range(1, berth_count + 1):
            vessel_approach = {
    ...
    def _print_summary(self, all_metrics: dict[str, list[CycleMetrics]]):
        ...
        print("\n" + "=" * 60, flush=True)
        print(f"  MULTI-AGENT SUMMARY ({berth_count} BERTH{'S' if berth_count > 1 else ''})", flush=True)
        print("=" * 60, flush=True)
        ...
        # Team state from HIVE — per berth
        if self.store:
            for berth_num in range(1, berth_count + 1):
                hold_id = self._berth_hold_id(berth_num)
                berth_id = self._berth_id(berth_num)
                ...
                if team_doc:
                    print(f"\n  TEAM STATE ({hold_id} / {berth_id}):", flush=True)
