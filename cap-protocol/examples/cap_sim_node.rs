...
/// Squad leader aggregation loop - runs every 5 seconds
async fn squad_leader_aggregation_loop(
    ...
    loop {
        ...
        // Note: This is a simplified implementation - production would use proper DQL queries
        let query = format!(
        ...
        // For now, we'll use an empty vector as placeholder
...
/// Platoon leader aggregation loop - runs every 5 seconds
async fn platoon_leader_aggregation_loop(
    ...
    loop {
        // Query squad summaries from Ditto
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check for MODE environment variable to enable hierarchical aggregation
    ...
    // Spawn hierarchical aggregation tasks based on role (Mode 4)
    if hierarchical_mode {
        let role = std::env::var("ROLE").unwrap_or_else(|_| "soldier".to_string());

        // Get DittoStore from backend for aggregation
        if let Some(ditto_backend) = backend.as_any().downcast_ref::<DittoBackend>() {
            match ditto_backend.get_ditto_store() {
                ...
                Err(e) => {
                    eprintln!("[{}] Failed to get DittoStore for aggregation: {}", node_id, e);
                }
            }
        }
    }
    ...
    let result = match mode.as_str() {
        ...
        "hierarchical" => {
            ...
            // The aggregation loops are already running in background tasks
...
async fn writer_mode(
    ...
    while start_time.elapsed() < test_duration {
        ...
        // Add public field for CAP filtering (Mode 3)
