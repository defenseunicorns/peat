...
use hive_protocol::network::PeerInfo;
...
async fn test_hierarchical_sync_soldiers_to_leader() {
    ...
    if std::env::var("CI").is_ok() && std::env::var("HIVE_E2E_HIERARCHICAL").is_err() {
        println!("⚠ Skipping hierarchical E2E test in CI (set HIVE_E2E_HIERARCHICAL=1 to enable)");
        return;
    }
