...
mod tests {
    ...
    async fn create_test_store() -> Result<NodeStore<DittoBackend>> {
        ...
        let app_id = std::env::var("HIVE_APP_ID")
            ...
            .map_err(|_| Error::storage_error("HIVE_APP_ID not set", "create_test_store", None))?;

        let shared_key = std::env::var("HIVE_SECRET_KEY")
            .or_else(|_| std::env::var("DITTO_SHARED_KEY"))
            .map_err(|_| {
                Error::storage_error("HIVE_SECRET_KEY not set", "create_test_store", None)
            })?;
    ...
    async fn test_get_operational_nodes() {
        ...
        store
            .store_state("worker_martinez_j", &operational)
            .await
            .unwrap();
